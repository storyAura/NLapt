...
+ hover cursor), the resizable 3-column QSplitter body, and splitter-size
persistence.
...
class TestSplitterBody:
    def test_body_is_splitter_with_three_widgets(self, window) -> None:
        splitter = window.body_splitter
        assert isinstance(splitter, QSplitter)
        assert splitter.orientation() == Qt.Orientation.Horizontal
        assert splitter.count() == 3
        assert splitter.widget(0) is window.file_panel
        assert splitter.widget(2) is window.tools_panel
    ...
    def test_side_panel_width_lock_released(self, window) -> None:
        for panel in (window.file_panel, window.tools_panel):
            assert panel.minimumWidth() == PANEL_MIN_W
            assert panel.maximumWidth() == PANEL_MAX_W
    ...
    def test_dragging_divider_changes_panel_widths(self, window) -> None:
        splitter = window.body_splitter
        left, middle, right = splitter.sizes()
        splitter.setSizes([left + 120, middle - 120, right])
        assert window.file_panel.width() == left + 120
        # The middle column stretches; the far panel keeps its width.
        assert window.tools_panel.width() == right
...
class TestSplitterPersistence:
    def test_round_trip(self, tmp_path) -> None:
        ...
        save_splitter_sizes([280, 700, 360], path)
        assert load_splitter_sizes(path) == [280, 700, 360]
    ...
    def test_saved_sizes_restore_side_widths(self, qtbot, controller, manager, tmp_path) -> None:
        ...
        assert win.tools_panel.width() == 380
    ...
    def test_close_persists_current_sizes(self, qtbot, controller, manager, tmp_path) -> None:
        ...
        win.show()
        qtbot.waitExposed(win)
        left, middle, right = win.body_splitter.sizes()
        win.body_splitter.setSizes([left + 40, middle - 40, right])
        win.close()
